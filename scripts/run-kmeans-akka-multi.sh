...
for (( WORKER=1; WORKER<$WORKERS; WORKER++ )); do
    ...
    ${JAVA} -classpath ${CLASSPATH} apgas.scala.examples.kmeans.KMeansAkkaDistributed ${POINTS} ${WORKERS} ${WORKER} &
done
...
${JAVA} -classpath ${CLASSPATH} apgas.scala.examples.kmeans.KMeansAkkaDistributed ${POINTS} ${WORKERS} 0
